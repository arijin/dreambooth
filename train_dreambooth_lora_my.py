...
from LightDataset import LightDataset
...
def main(args):
    ...
    dataset_path = "/data_4/jyj/datasets/SinLight"
    dataset = LightDataset(dataset_path, transform, tokenizer)
    ...
    if args.train_text_encoder:
        ...
        unet_lora_layers, optimizer, train_dataloader, lr_scheduler, train_loader = accelerator.prepare(
            unet_lora_layers, optimizer, train_dataloader, lr_scheduler, train_loader
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            #     truncation=True,
            #     return_tensors="pt").input_ids # [64, 77]
            # batch["input_ids"] = text_input_ids.to(accelerator.device)
            batch["pixel_values"] = batch["pixel_values"].to(accelerator.device)
            batch["input_ids"] = batch["input_ids"].to(accelerator.device)
            # batch["pixel_values"] = torch.randn(64, 3, 96, 32).to(accelerator.device)
            # batch["input_ids"] = torch.randint(0, 100, (64, 77)).to(accelerator.device)
